...
function PasswordStrengthBar({ password }: { password: string }) {
  ...
  return (
    <div className="mt-2 space-y-1.5">
      ...
      <p className={`text-xs font-medium ${levels[score]?.textColor}`}>
        {levels[score]?.label}
      </p>
    </div>
  );
...
function PainelEsquerdo({
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(145deg, #0044AA 0%, #0070E0 50%, #2299FF 100%)",
      }}
    >
      <div
...
export default function RecuperarSenha() {
  ...
  const handleCodigoChange = (index: number, value: string) => {
    // Aceita somente dígitos e permite colar o código inteiro
    const cleaned = value.replace(/\D/g, "");

    if (cleaned.length > 1) {
      // Colagem: distribui pelos campos
  ...
  // ── Configuração de conteúdo do painel esquerdo por etapa ────────────────
  const painelConfig = {
    email: {
      ...
      items: [
        ...
        "Valide o código para prosseguir com segurança",
        "Crie uma nova senha e retome o acesso",
      ],
    },
    codigo: {
    ...
    sucesso: {
      ...
      items: [
        ...
        "Contate o suporte em caso de problemas",
      ],
    },
  };
  ...
  const stepLabels = ["E-mail", "Código", "Nova senha", "Concluído"];
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-white px-8 py-12 relative">
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="lg:hidden flex items-center gap-2 mb-8 justify-center">
            ...
            <span className="font-bold text-gray-800 text-lg">GovIT Manager</span>
          </div>
            ...
            <div className="flex items-center gap-0 mb-8">
              {stepLabels.slice(0, 3).map((label, i) => (
                <div key={i} className="flex items-center flex-1">
                  ...
                  {i < 2 && (
                    <div
                      className={`flex-1 h-0.5 mb-4 mx-1 rounded-full transition-all duration-300 ${
            ...
            <>
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">
                  Informe o e-mail associado à sua conta. Enviaremos um código de verificação.
                </p>
              </div>
              ...
              <form onSubmit={handleEnviarEmail} className="space-y-5">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full py-2.5 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-200 shadow-md ${
                    isLoading ? "bg-blue-400 cursor-not-allowed shadow-none" : "bg-blue-600 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 active:scale-[0.98]"
                  }`}
                >
                  {isLoading ? (
                    <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="30 70" /></svg>Enviando...</>
                  ) : (
                    <><Send className="w-4 h-4" />Enviar código de verificação</>
                  )}
                </button>
              </form>
            </>
          ...
          {step === "codigo" && (
            <>
              ...
              <div className="text-center mb-2">
                ...
                <p className="text-blue-600 font-semibold text-sm mt-1 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 inline-block">
                  {email}
                </p>
              </div>

              <form onSubmit={handleVerificarCodigo} className="mt-7 space-y-6">
                {/* Inputs do código */}
                <div>
                  ...
                  <div className="flex gap-2 justify-center">
                    {codigo.map((digit, i) => (
                      <input
                        ...
                        className={`w-11 h-13 text-center text-lg font-bold border rounded-xl outline-none transition-all duration-200 bg-gray-50 focus:bg-white ${
                          digit
                            ? "border-blue-400 bg-blue-50 text-blue-700 focus:ring-2 focus:ring-blue-500/20"
                            : "border-gray-200 text-gray-900 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600"
                        }`}
                        style={{ height: "52px" }}
                      />
                    ))}
                ...
                <button
                  type="submit"
                  disabled={isLoading || codigoCompleto.length < 6}
                  className={`w-full py-2.5 rounded-xl text-white font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-200 shadow-md ${
                    isLoading || codigoCompleto.length < 6
                      ? "bg-blue-400 cursor-not-allowed shadow-none"
                      : "bg-blue-600 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 active:scale-[0.98]"
                  }`}
                >
                  {isLoading ? (
                    <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="30 70" /></svg>Verificando...</>
                  ) : (
          ...
          {step === "senha" && (
            <>
              ...
              <div className="text-center mb-8">
                <h1 className="text-2xl font-bold text-gray-900 mb-1">Crie uma nova senha</h1>
                <p className="text-gray-500 text-sm leading-relaxed">
                  Sua identidade foi confirmada. Defina uma senha segura para sua conta.
                </p>
              </div>
              ...
              <form onSubmit={handleRedefinirSenha} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-4 py-2.5 pr-11 border rounded-xl focus:ring-2 outline-none transition-all text-sm bg-gray-50 focus:bg-white placeholder:text-gray-400 ${
                  ...
                  {confirmarSenha && novaSenha !== confirmarSenha && (
                    <p className="mt-1.5 text-xs text-red-500 font-medium">As senhas não coincidem.</p>
                  )}
                  {confirmarSenha && novaSenha === confirmarSenha && (
                ...
                >
                  {isLoading ? (
                    <><svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" strokeDasharray="30 70" /></svg>Redefinindo...</>
                  ) : (
          ...
          {step === "sucesso" && (
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm leading-relaxed mb-8">
                Sua senha foi alterada com sucesso. Você já pode acessar o sistema com suas novas credenciais.
              </p>

              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-8 text-left">
                <p className="text-blue-700 text-xs font-medium mb-1">🔒 Dica de segurança</p>
                <p className="text-blue-600 text-xs leading-relaxed">
                  Nunca compartilhe sua senha com terceiros. Caso suspeite de acessos não autorizados, contate o suporte de TI imediatamente.
                </p>
              </div>
          ...
          {/* Voltar para o login (etapas 1 e 3) */}
          ...
          <div className="mt-10 pt-6 border-t border-gray-100">
            <p className="text-center text-xs text-gray-400">
              © {new Date().getFullYear()} Departamento de Tecnologia da Informação.
              <br />
