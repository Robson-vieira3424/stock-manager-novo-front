...
export default function FormAtualizarManutencao({
  ...
  const [pecasUsadas, setPecasUsadas] = useState<
    { id: number; name: string; qtd: number }[]
  >([]);
    ...
    if (manutencao && open) {
      form.reset({
        ...
        dataPrevisao: manutencao.dataPrevisao
          ? new Date(manutencao.dataPrevisao)
          : undefined,
  ...
  const adicionarPeca = () => {
    ...
    const pecaDb = pecasDisponiveis.find(
      (p) => p.id.toString() === pecaSelecionadaId,
    );
    if (!pecaDb) return;
    setPecasUsadas((prev) => {
      ...
      if (existe)
        return prev.map((p) =>
          p.id === pecaDb.id ? { ...p, qtd: p.qtd + quantidadePeca } : p,
        );
      return [
        ...prev,
        { id: pecaDb.id, name: pecaDb.name, qtd: quantidadePeca },
      ];
    });
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const payload = {
        ...
        pecas:
          data.utilizouPeca === "sim"
            ? pecasUsadas.map((p) => ({ id: p.id, quantity: p.qtd }))
            : [],
      };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[650px] max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <strong className="text-slate-700">
              {manutencao.pc?.patrimonio}
            </strong>{" "}
            · Técnico:{" "}
            <strong className="text-slate-700">
              {manutencao.nomeTecnico || "Não atribuído"}
            </strong>
          </DialogDescription>
        </DialogHeader>
        ...
        <form
          ...
          className="space-y-6 py-4 overflow-y-auto px-4"
        >
              ...
              <div className="flex flex-col md:flex-row gap-3 items-end">
                <div className="flex flex-col gap-2 flex-1">
                  ...
                  <Select
                    ...
                    onValueChange={setPecaSelecionadaId}
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={
                          loading ? "Carregando..." : "Escolha uma peça..."
                        }
                      />
                    </SelectTrigger>
                    <SelectContent>
